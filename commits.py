...
def get_commits(repo):
    ...
    res = requests.get('https://api.github.com/repos/ownrecipes/%s/tags' % repo)
    commit = res.json()[0].get('commit').get('sha')

    # Get the date of the last commit from the last release
    res = requests.get('https://api.github.com/repos/ownrecipes/%s/commits/%s' % (repo, commit))
    ...
    res = requests.get('https://api.github.com/repos/ownrecipes/%s/commits?since=%s' % (repo, date))
    return [re.get('commit').get('message') for re in res.json()]


if __name__ == '__main__':
    messages = []
    repos = ['ownrecipes-nginx', 'ownrecipes-api', 'ownrecipes-web', 'ownrecipes']
    for r in repos:
